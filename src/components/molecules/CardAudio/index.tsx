  ...
  return (
    ...
    >
      <Image
        ...
        alt={data.title}
        title={data.title}
        className="h-3/4 w-56 rounded object-cover shadow-lg sm:h-3/4 lg:h-2/3"
        width={380}
        height={380}
      />
      <div className="mt-2 flex w-full flex-col items-start justify-center">
        <strong className="w-full truncate text-xl font-semibold sm:text-base">
          {data.title}
        </strong>
        <span className="text-md w-full truncate text-zinc-500 sm:text-sm">
          {data.description}
        </span>
      </div>
    </button>
